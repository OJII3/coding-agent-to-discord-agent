...
コードレビューの orchestrator スキル。3つのレビューサブエージェント（`.claude/agents/`）を並列起動し、結果を統合して **即修正** または **Issue 起票** に振り分ける。
...
以下の 3 エージェントを **Agent ツール（general-purpose）で並列に** 起動する。各エージェントには Phase 1 で取得した diff 内容と変更ファイル一覧を渡す。
...
- Agent ツールの `name` に各エージェント名を設定し、3つ並列で呼び出す
- `prompt` に `.claude/agents/{name}.md` の内容全文と diff 内容を含める
- `model: sonnet` を指定する

## Phase 3: 結果統合と振り分け

3 エージェントの報告を受け取ったら、全指摘を統合し、以下の 2 カテゴリに振り分ける。
